...
class MLPProjection(nn.Module):
    def __init__(self, inbits, outbits, backbonemodel):
        super(MLPProjection, self).__init__()
        self.backbonemodel = backbonemodel
        self.layers = nn.Sequential(
            nn.Linear(inbits, 128),
            nn.ReLU(),
            nn.Linear(128, outbits)
        )
...
class mobilenet_v3_largehashing(nn.Module):
    def __init__(self, inchannel=2, bits=1024):
        ...
        self.model.classifier = nn.Linear(self.model.classifier[1].in_features, bits)
