    ...
    if istrain:
        for i in tqdm.tqdm(range(train_num)):
            for j in range(sample_num):
                ...
                r_img = np.array(ImageOps.autocontrast(
                    Image.open(os.path.join(r_img_path, "%03d_" % (i + 1) + "%02d.jpg" % (j + 1)))))

                b_img = np.array(ImageOps.autocontrast(
                    Image.open(os.path.join(b_img_path, "%03d_" % (i + 1) + "%02d.jpg" % (j + 1)))))

                n_img = np.array(ImageOps.autocontrast(
                    Image.open(os.path.join(n_img_path, "%03d_" % (i + 1) + "%02d.jpg" % (j + 1)))))
    ...
    #                 labels.append(i)
        ...
        for i in tqdm.tqdm(range(train_num)):
            for j in range(sample_num, 6):
                ...
                imgprint = np.dstack((r_img, b_img))
                imgvein = np.dstack((n_img, n_img))
...
#         transforms.Resize((224, 224)),# if resnet
        ...
        if self.training:
            print('\n...... Train files loading\n')
            if ds == 'polyu':
                self.vein_list,self.prints_list, self.labels= part_init_polyu(istrain=True,train_ratio = train_ratio,sample_ratio = sample_ratio)
            elif ds == 'tjppv':
            ...
            print('\n...... Test files loading\n')
            if ds == 'polyu':
                self.vein_list,self.prints_list, self.labels = part_init_polyu(istrain=False,train_ratio = train_ratio,sample_ratio = sample_ratio)
            elif ds == 'tjppv':
                self.vein_list,self.prints_list, self.labels = part_init_tjppv(istrain=False,train_ratio = train_ratio,sample_ratio = sample_ratio)
            elif ds == 'iitd':
                self.vein_list,self.prints_list, self.labels = part_init_iitd(istrain=False,train_ratio = train_ratio,sample_ratio = sample_ratio)
            elif ds == 'casiam':
                self.vein_list,self.prints_list, self.labels = part_init_casiam(istrain=False,train_ratio = train_ratio,sample_ratio = sample_ratio)
            else:
                print('wrong DS',ds)
            print('\nTest files loaded ......\n')
