...
def eval_eer_hashcenter(FEATS_prints_binay, criterion, clsses=500, sampesCls=4):
    ...
    for i in tqdm.tqdm(range(totalsamples)):  # 50 11
        ...
        wronghash = criterion.label2center(
            np.expand_dims(one_hot_embedding(getRandPerson(exclude=i // sampesCls, totalcls=clsses), clsses),
                           axis=0)) > 0
...
def eval_eer_cross(FEATS_prints, FEATS_veins, clsses=500, sampesCls=4, disfun='cossim'):
    ...
    for i in tqdm.tqdm(range(totalsamples)):  # 50 11
        if disfun == 'cossim':
            ...
            a = hamming_sim(FEATS_prints[i, :], FEATS_veins[i, :])  # RED vs NIR same
        pred_scores_cross.append(a)
        gt_label_cross.append(True)

        if disfun == 'cossim':
            impo = cossim(FEATS_prints[i, :],
                          FEATS_veins[getRandPerson(exclude=i // sampesCls, totalcls=clsses) * sampesCls, :])
        else:
            impo = hamming_sim(FEATS_prints[i, :],
                               FEATS_veins[getRandPerson(exclude=i // sampesCls, totalcls=clsses) * sampesCls, :])
