...
try:
	visualize_graph(model, writer, input_size=(1, 3, 140, 140))
except:
...
def train(epoch):
    ...
    for batch_idx, img in enumerate(train_loader):
        r = torch.unsqueeze(img[0], 1).to(device, dtype=torch.float)
        b = torch.unsqueeze(img[1], 1).to(device, dtype=torch.float)
        n = torch.unsqueeze(img[2], 1).to(device, dtype=torch.float)
        label = img[3].to(device)
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, img in enumerate(test_loader):
            ...
            output = model(inputs)
            test_loss.update(F.cross_entropy(output, label, reduction='mean').item(), label.size(0))
            prec1, = accuracy(output, label) # test precison in one batch
            acc.update(prec1.item(), label.size(0))
